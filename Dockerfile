...
# 创建启动脚本
RUN echo 'whoami ' >>/start.sh
RUN echo 'cd ' >>/start.sh
RUN echo "su -l -c  'vncserver :2000 -geometry 1280x800' "  >>/start.sh
RUN echo 'cd /noVNC-1.2.0' >>/start.sh
RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/start.sh
